...
export function timeSince( date ) {
	...
	if ( date > currentDate ) {
		return __( 'Online Recently', 'it-l10n-ithemes-security-pro' );
	}
	...
	if ( interval > 1 ) {
		return sprintf(
			...
			__( '%s years', 'it-l10n-ithemes-security-pro' ),
			Math.floor( interval )
		);
	}
	...
	if ( interval > 1 ) {
		return sprintf(
			...
			__( '%s months', 'it-l10n-ithemes-security-pro' ),
			Math.floor( interval )
		);
	}
	...
	if ( interval > 1 ) {
		return sprintf(
			...
			__( '%s days', 'it-l10n-ithemes-security-pro' ),
			Math.floor( interval )
		);
	}
	...
	if ( interval > 1 ) {
		return sprintf(
			...
			__( '%s hours', 'it-l10n-ithemes-security-pro' ),
			Math.floor( interval )
		);
	}
	interval = seconds / 60;
	if ( interval > 1 ) {
		return sprintf(
			...
			__( '%s minutes', 'it-l10n-ithemes-security-pro' ),
			Math.floor( interval )
		);
	}
	return sprintf(
		/* translators: 1. number of seconds since */
		__( '%s seconds', 'it-l10n-ithemes-security-pro' ),
		Math.floor( interval )
	);
}
