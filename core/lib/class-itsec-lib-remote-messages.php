...
class ITSEC_Lib_Remote_Messages {
	...
	public static function fetch() {
		...
		if ( ! $data ) {
			return new WP_Error( 'empty_body', __( 'Empty response body.', 'it-l10n-ithemes-security-pro' ) );
		}

		$json = json_decode( $data, true );

		if ( ! $json ) {
			return new WP_Error( 'invalid_json', __( 'Invalid json response.', 'it-l10n-ithemes-security-pro' ) );
		}
