...
class ITSEC_Lib_Feature_Flags {
	...
	public static function register_flag( $name, $args = array() ) {
		if ( ! preg_match( '/^\w+$/', $name ) ) {
			return new WP_Error( 'invalid_flag_name', __( 'Invalid flag name.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	public static function get_reason( $flag ) {
		if ( ! $config = self::get_flag_config( $flag ) ) {
			return [ 'unknown', __( 'Unknown flag', 'it-l10n-ithemes-security-pro' ) ];
		}
		...
		if ( defined( 'ITSEC_FF_' . $flag ) ) {
			return [ 'constant', __( 'Manually configured with a constant.', 'it-l10n-ithemes-security-pro' ) ];
		}

		if ( ! empty( $config['disabled'] ) ) {
			return [ 'remote', __( 'Remotely disabled by SolidWP.', 'it-l10n-ithemes-security-pro' ) ];
		}

		$rates = ITSEC_Modules::get_setting( 'feature-flags', 'rates' );

		if ( isset( $rates[ $flag ] ) ) {
			return [ 'rollout', __( 'Gradually rolling out.', 'it-l10n-ithemes-security-pro' ) ];
		}

		return [ 'setting', __( 'Configured on the Feature Flags page.', 'it-l10n-ithemes-security-pro' ) ];
	}
