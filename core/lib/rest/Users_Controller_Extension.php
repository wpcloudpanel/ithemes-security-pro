	...
	public function register_collection_params( $params ) {
		...
		$params['solid_password_strength'] = [
			...
			'items' => [
				...
				'enum'      => [ 0, 1, 2, 3, 4 ],
				'enumNames' => [
					_x( 'Very Weak', 'password strength', 'it-l10n-ithemes-security-pro' ),
					_x( 'Weak', 'password strength', 'it-l10n-ithemes-security-pro' ),
					_x( 'Medium', 'password strength', 'it-l10n-ithemes-security-pro' ),
					_x( 'Strong', 'password strength', 'it-l10n-ithemes-security-pro' ),
				]
			],
		];
		...
		if ( \ITSEC_Modules::is_active( 'two-factor' ) ) {
			...
			$enumNames = [
				__( 'Configured Two-Factor', 'it-l10n-ithemes-security-pro' ),
				__( 'Not Configured Two-Factor', 'it-l10n-ithemes-security-pro' ),
			];

			if ( \ITSEC_Core::get_install_type() === 'pro' && \ITSEC_Modules::get_setting( 'two-factor', 'protect_user_group' ) ) {
				$enum[]      = 'enforced-not-configured';
				$enumNames[] = __( 'Two-Factor Enforced', 'it-l10n-ithemes-security-pro' );
			}
		...
		register_rest_field( 'user', 'solid_2fa', [
			...
			'schema'       => [
				...
				'enumNames' => [
					__( 'Configured Two-Factor', 'it-l10n-ithemes-security-pro' ),
					__( 'Not Configured Two-Factor', 'it-l10n-ithemes-security-pro' ),
					__( 'Two-Factor Enforced', 'it-l10n-ithemes-security-pro' ),
				],
				'context'   => [ 'edit' ],
				'readonly'  => true,
			],
