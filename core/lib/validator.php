...
abstract class ITSEC_Validator {
			...
			if ( gettype( $default ) !== gettype( $value ) ) {
				$this->add_error(
					new WP_Error(
						...
						sprintf(
							__( 'A validation function for %1$s received data that does not match the expected data type for the %2$s entry. A data type of %3$s was expected, but a data type of %4$s was received.', 'it-l10n-ithemes-security-pro' ),
							$id,
							$name,
							gettype( $default ),
							gettype( $value )
	...
	protected function validate_user_groups( $name, $var ) {
		...
		$error = wp_sprintf(
			_n( 'The following entry in %1$s is invalid: %2$l', 'The following entries in %1$s are invalid: %2$l', count( $invalid_user_groups ), 'it-l10n-ithemes-security-pro' ),
			$name,
			$invalid_user_groups
		);
