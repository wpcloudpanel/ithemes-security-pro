	...
	public function handle_ajax_request() {
		...
		if ( ! empty( $_POST['method'] ) && 'handle_logs_migration' === $_POST['method'] ) {
			...
			if ( false === check_ajax_referer( 'itsec-logs-nonce', 'nonce', false ) ) {
				ITSEC_Response::add_error( new WP_Error( 'itsec-logs-page-failed-nonce', __( 'A security check failed, preventing the request from completing as expected. Please try reloading the page and trying again.', 'it-l10n-ithemes-security-pro' ) ) );
				...
				$details = array(
					...
					'host'        => array(
						'header'  => esc_html__( 'IP Address', 'it-l10n-ithemes-security-pro' ),
...
?>
	<fieldset class="screen-options">
		<legend><?php esc_html_e( 'Pagination', 'it-l10n-ithemes-security-pro' ); ?></legend>
		<label for="itsec_logs_page_entries_per_page"><?php esc_html_e( 'Number of items per page:', 'it-l10n-ithemes-security-pro' ); ?></label>
		<?php $form->add_number( 'per_page', array( 'step' => 1, 'min' => 1, 'max' => 999, 'maxlength' => 3 ) ); ?>
	</fieldset>

	<fieldset>
		<legend><?php esc_html_e( 'View Mode', 'it-l10n-ithemes-security-pro' ); ?></legend>
	...
	<p class="submit">
		<?php $form->add_submit( 'apply', __( 'Apply', 'it-l10n-ithemes-security-pro' ) ); ?>
	</p>
...
?>
	<div class="wrap">
		<h1>
			...
			<a href="<?php echo esc_url( apply_filters( 'itsec_support_url', 'https://go.solidwp.com/org-support-link' ) ); ?>" class="page-title-action"><?php _e( 'Support', 'it-l10n-ithemes-security-pro' ); ?></a>
		</h1>
