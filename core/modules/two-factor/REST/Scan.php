	...
	public function prepare_item_for_response( $item, $request ) {
		$response = new \WP_REST_Response( [
			...
			/* translators: 1. User display name. */
				__( '%s has administrator capabilities, but does not have Two-Factor enabled.', 'it-l10n-ithemes-security-pro' ),
				$item->display_name
			),
			'severity'    => 'medium',
		...
		if ( $response->get_data()['muted'] ) {
			...
			$response->add_link(
				...
				[
					'title' => __( 'Remind User', 'it-l10n-ithemes-security-pro' )
				]
			);
			$response->add_link(
