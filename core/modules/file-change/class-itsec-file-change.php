...
class ITSEC_File_Change {
	...
	/**
	 * When Solid Security writes to a file, store the file's hash so the change is not seen as unexpected.
	 *
	 * @param string $file
	 */
	...
	public function register_notification_strings() {
		return array(
			'label'       => __( 'File Change', 'it-l10n-ithemes-security-pro' ),
			'description' => sprintf(
				__( 'The %1$sFile Change Detection%2$s module will email a file scan report after changes have been detected.', 'it-l10n-ithemes-security-pro' ),
				ITSEC_Core::get_link_for_settings_route( ITSEC_Core::get_settings_module_route( 'file-change' ) ),
				'</a>'
			),
			'subject'     => __( 'File Change Warning', 'it-l10n-ithemes-security-pro' ),
		);
	}
