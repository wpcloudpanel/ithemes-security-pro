...
class ITSEC_REST_Core_Admin_Notices_Controller extends WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			return new WP_Error( 'not_logged_in', esc_html__( 'You must be logged in to view notices.', 'it-l10n-ithemes-security-pro' ), array(
	...
	public function update_item_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			return new WP_Error( 'not_logged_in', esc_html__( 'You must be logged in to view notices.', 'it-l10n-ithemes-security-pro' ) );
		}

		if ( ! $notice = $this->get_notice_for_request( $request ) ) {
			return new WP_Error( 'notice_not_found', esc_html__( 'Notice not found.', 'it-l10n-ithemes-security-pro' ), array(
				'status' => WP_Http::NOT_FOUND,
			) );
		}

		if ( ! array_key_exists( $request['action'], $notice->get_actions() ) ) {
			return new WP_Error( 'action_not_found', esc_html__( 'Action not found.', 'it-l10n-ithemes-security-pro' ), array(
				'status' => WP_Http::NOT_FOUND,
			) );
		}
