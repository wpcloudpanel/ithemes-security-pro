...
class ITSEC_Core_Admin implements Runnable {
	...
	public function add_plugin_meta_links( $meta ) {
		...
		$meta[] = '<a href="' . $link . '" target="_blank" rel="noopener noreferrer">' . __( 'Get Support', 'it-l10n-ithemes-security-pro' ) . '</a>';
			...
			if ( end( $plugin_file_array ) === end( $core_plugin_file_array ) ) {
				echo '
					...
					<div class="itsec-plugin-rebranding-container">
						<h4 class="itsec-rebranding-header">' . __( "iThemes Security is now Solid Security: More Security, Better UIs, and Improved Features", "it-l10n-ithemes-security-pro" ) . '</h4>
						<p>' . __( "We have been working hard for almost a year to bring you incredible new features in the form of our new and improved brand: SolidWP.", "it-l10n-ithemes-security-pro" ) . '</p>
						<a href="' . $branding_link . '">' . __( 'Learn More About Solid Security and SolidWP', 'it-l10n-ithemes-security-pro') . '</a>
	...
	public function process_rebranding_dismissal() {
		...
		if ( $result ) {
			...
			return new WP_Error( 'itsec-admin-notices.updating_site_option_failed', esc_html__( 'Could not dismiss action.', 'it-l10n-ithemes-security-pro' ) );
		}
	}
