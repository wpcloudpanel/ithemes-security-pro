	...
	public function register_tools( Tools_Registry $registry ) {
			...
			public function run( array $form = [] ): Result {
				...
				if ( ! file_exists( WP_CONTENT_DIR . '/plugins/' . $basename ) ) {
					return Result::error( new WP_Error(
						...
						__( 'Could not determine the correct path to load Solid Security.', 'it-l10n-ithemes-security-pro' )
					) );
				}
				...
				return Result::success()->add_success_message( __( 'Created MU-Plugin loader.', 'it-l10n-ithemes-security-pro' ) );
			}
		...
		$registry->register( new class( 'remove-mu-plugin', ITSEC_Modules::get_config( 'core' ) ) extends Config_Tool {
			public function run( array $form = [] ): Result {
				...
				return Result::success()->add_success_message( __( 'Removed MU-Plugin loader.', 'it-l10n-ithemes-security-pro' ) );
			}
