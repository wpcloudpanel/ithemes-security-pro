	...
	try {
		...
		yield createNotice(
			...
			sprintf(
				...
				__( 'Error when adding card to dashboard: %s', 'it-l10n-ithemes-security-pro' ),
				e.message
			)
		);
...
export function* saveDashboardCard( dashboardId, card ) {
	...
	try {
		...
		yield createNotice(
			...
			sprintf(
				/* translators: 1. Error message. */
				__( 'Error when saving dashboard card: %s', 'it-l10n-ithemes-security-pro' ),
				e.message
			)
		);
...
export function* removeDashboardCard( dashboardId, card, optimistic = true ) {
	...
	try {
		...
		yield createNotice(
			...
			sprintf(
				/* translators: 1. Error message. */
				__( 'Error when removing card from dashboard: %s', 'it-l10n-ithemes-security-pro' ),
				e.message
			)
		);
...
export function* refreshDashboardCards( dashboardId ) {
	...
	try {
		...
		yield createNotice(
			...
			sprintf(
				/* translators: 1. Error message */
				__( 'Refreshing dashboard data failed: %s', 'it-l10n-ithemes-security-pro' ),
				e.message
			),
			{ autoDismiss: true }
		);
...
export function* callDashboardCardRpc( cardId, href, data ) {
	...
	try {
		...
		yield createNotice(
			...
			sprintf(
				/* translators: 1. Error message. */
				__( 'Error when performing card action: %s', 'it-l10n-ithemes-security-pro' ),
				e.message
			)
		);
