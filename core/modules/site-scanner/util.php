...
class ITSEC_Site_Scanner_Util {
		...
		foreach ( $parts as $part ) {
			switch ( $part ) {
				case 'found-malware':
					...
					break;
				case 'on-blacklist':
					$part_labels[] = esc_html__( 'On Blocklist', 'it-l10n-ithemes-security-pro' );
					break;
	...
	public static function is_log_item_valid_scan( $entry ) {
		if ( ! $entry ) {
			return new \WP_Error( 'itsec_site_scanner_factory_log_not_found', __( 'Could not find a log item with that id.', 'it-l10n-ithemes-security-pro' ) );
		}

		if ( 'site-scanner' !== $entry['module'] ) {
			return new \WP_Error( 'itsec_site_scanner_factory_invalid_log_item', __( 'Log item does not belong to the Site Scanner module.', 'it-l10n-ithemes-security-pro' ) );
		}

		if ( in_array( $entry['type'], [ 'process-start', 'process-update', 'process-stop' ], true ) ) {
			return new \WP_Error( 'itsec_site_scanner_factory_invalid_log_item_type', __( 'Log item is of the incorrect type.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	public static function validate_scan_auth_token( $jwt ) {
		...
		if ( $decoded->grant !== self::GRANT ) {
			return new WP_Error( 'itsec_site_scanner_invalid_grant', __( 'Malformed token.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	public static function mute_issue( $issue_id, array $args = [] ) {
		...
		if ( ! $vulnerability ) {
			return new WP_Error(
				...
				__( 'Vulnerability not found.', 'it-l10n-ithemes-security-pro' ),
				[
					'id' => $issue_id,
				]
			);
		...
		if ( $vulnerability->is_muted() ) {
			return new WP_Error(
				...
				__( 'Issue already muted.', 'it-l10n-ithemes-security-pro' ),
				[
					'id' => $issue_id,
				]
			);
	...
	public static function unmute_issue( $issue_id ) {
		...
		if ( ! $vulnerability->is_muted() ) {
			return new WP_Error(
				...
				__( 'Issue already not muted.', 'it-l10n-ithemes-security-pro' ),
				[
					'id' => $issue_id,
				]
			);
