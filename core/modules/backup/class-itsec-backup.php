...
class ITSEC_Backup {
	...
	public function do_backup( $one_time = false ) {
		...
		switch ( $this->settings['method'] ) {
			case 'both':
				$message = __( 'Backup complete. The backup was sent to the selected email recipients and was saved locally.', 'it-l10n-ithemes-security-pro' );
				break;
			case 'email':
				$message = __( 'Backup complete. The backup was sent to the selected email recipients.', 'it-l10n-ithemes-security-pro' );
				break;
			default:
				$message = __( 'Backup complete. The backup was saved locally.', 'it-l10n-ithemes-security-pro' );
				break;
		}
	...
	private function send_mail( $file ) {
		...
		$mail->add_header(
			esc_html__( 'Database Backup', 'it-l10n-ithemes-security-pro' ),
			sprintf( esc_html__( 'Site Database Backup for %s', 'it-l10n-ithemes-security-pro' ),
				'<b>' . date_i18n( get_option( 'date_format' ) ) . '</b>' ),
			false,
			esc_html__( 'Attached is the database backup file for your site.', 'it-l10n-ithemes-security-pro' ),
		);
	...
	public function notification_strings() {
		return array(
			'label'       => __( 'Database Backup', 'it-l10n-ithemes-security-pro' ),
			'description' => sprintf(
				__( 'The %1$sDatabase Backup%2$s module will send a copy of any backups to the email addresses listed below.', 'it-l10n-ithemes-security-pro' ),
				ITSEC_Core::get_link_for_settings_route( ITSEC_Core::get_settings_module_route( 'backup' ) ),
				'</a>'
			),
			'subject'     => __( 'Database Backup', 'it-l10n-ithemes-security-pro' ),
		);
	}
