...
class ITSEC_Notify {
	...
	public function notification_strings() {
		$description = esc_html__( 'During periods of heavy attack, Solid Security can generate a LOT of email.', 'it-l10n-ithemes-security-pro' );

		if ( ITSEC_Core::is_pro() ) {
			$features = esc_html__( 'The Security Digest reduces the number of emails sent so you can receive a summary of lockouts, file change detection scans, and privilege escalations.', 'it-l10n-ithemes-security-pro' );
		} else {
			$features = esc_html__( 'The Security Digest reduces the number of emails sent so you can receive a summary of lockouts and file change detection scans.', 'it-l10n-ithemes-security-pro' );
		}
	...
	public function send_daily_digest( $sent, $last_sent, $data ) {
		...
		$mail->add_header(
			...
			$banner_title,
			false,
			sprintf( esc_html__( 'The following is a summary of security related activity on your site: %s', 'it-l10n-ithemes-security-pro' ), '<b>' . $mail->get_display_url() . '</b>' )
		);
