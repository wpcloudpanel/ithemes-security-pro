...
function itsec_recaptcha_get_last_error_notice() {
	return sprintf(
		esc_html__( 'The reCAPTCHA settings for Solid Security are invalid. %1$s Bots will not be blocked until %2$sthe reCAPTCHA settings%3$s are set properly.', 'it-l10n-ithemes-security-pro' ),
		esc_html( ITSEC_Modules::get_setting( 'recaptcha', 'last_error' ) ),
		ITSEC_Core::get_link_for_settings_route( ITSEC_Core::get_settings_module_route( 'recaptcha' ) ),
		'</a>'
	);
}
