...
class Ithemes_Updater_Server {
	...
	public static function request( $action, $query = array(), $data = array() ) {
		...
		if ( ! self::is_valid_response( $response ) ) {
			...
			$message = __( 'Invalid server response.', 'it-l10n-ithemes-security-pro' );
			if ( ! is_wp_error( $response ) ) {
				$message = wp_remote_retrieve_response_code( $response ) . ': ' . wp_remote_retrieve_response_message( $response );
			}

			// Translators: %s is the error message.
			return new WP_Error( 'http_request_failed', sprintf( __( 'The server was unable to be contacted. (%s)', 'it-l10n-ithemes-security-pro' ), $message ), $response );
		}
		...
		if ( ! empty( $body['error'] ) ) {
			...
			return new WP_Error( $body['error']['type'], sprintf( __( 'An error occurred when communicating with the SolidWP update server: %1$s (%2$s)', 'it-l10n-ithemes-security-pro' ), $body['error']['message'], $body['error']['code'] ) );
		}
